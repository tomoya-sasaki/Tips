...
for (i in 1:nn){
  result[i,1] <-  SampleMu(databox2, theta0, 10, J)
}
proc.time() - t
...
SampleSigmaMiss <- function(data, theta, J, n){
  ...
  for (j in 1:J){
    ...
    theta_j <- theta[j]
    for (i in 1:n_j){
      temp_j <- temp_j + (data_j[i] - theta_j)^2 # you need temp_j + (data_j[i] - theta_j)^2
    }
    temp <- temp + temp_j 
  }
...
result <- data.frame(parameter = rep(NA, nn))
for (i in 1:nn){
  result[i,1] <-  SampleSigmaMiss(databox2, theta0, mu0, J)
}
proc.time() - t
...
databox3 <- data.frame(theta1 = rep(NA, n), theta2 = rep(NA, n) , theta3 = rep(NA, n), theta4 = rep(NA, n), mu = rep(NA, n), sigma = rep(NA, n), tau = rep(NA, n))
```
...
for (i in 1:n){
  ...
  theta_temp <- SampleThetaMiss(databox2, mu_temp, tau_temp, sigma_temp, J)
